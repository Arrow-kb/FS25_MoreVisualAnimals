...
function VisualAnimalsDialog.show()
    ...
    if VisualAnimalsDialog.INSTANCE ~= nil then
        ...
        local maxHusbandries = MVA_AnimalClusterHusbandry.MAX_HUSBANDRIES
...
function VisualAnimalsDialog.new(target, customMt)
    ...
    local self = YesNoDialog.new(target, customMt or visualAnimalsDialog_mt)

    self.areButtonsDisabled = false
    self.recommendedAnimals = 8
...
function VisualAnimalsDialog:onYes()
    ...
    if maxHusbandries ~= newMaxHusbandries then
        ...
        MVA_AnimalClusterHusbandry.MAX_HUSBANDRIES = newMaxHusbandries
        for _, clusterHusbandry in ipairs(husbandrySystem.clusterHusbandries) do
            clusterHusbandry.nextUpdateClusters = clusterHusbandry.placeable.spec_husbandryAnimals.clusterSystem:getClusters()
            clusterHusbandry:updateVisuals()
        end

        createFolder(modSettingsDirectory)

        local xmlFile = XMLFile.create("visualAnimalsCap", modSettingsDirectory .. "visualAnimals.xml", "settings")

        if xmlFile ~= nil then

            xmlFile:setInt("settings#limit", newMaxHusbandries)
